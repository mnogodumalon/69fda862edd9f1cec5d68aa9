...
import type { GluehweinErlebnis, CreateGluehweinErlebnis } from '@/types/app';
...
export class LivingAppsService {
  // --- GLUEHWEIN_ERLEBNIS ---
  static async getGluehweinErlebnis(): Promise<GluehweinErlebnis[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GLUEHWEIN_ERLEBNIS}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as GluehweinErlebnis[];
    return enrichLookupFields(records, 'gluehwein_erlebnis');
  }
  static async getGluehweinErlebni(id: string): Promise<GluehweinErlebnis | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GLUEHWEIN_ERLEBNIS}/records/${id}`);
    const record = { record_id: data.id, ...data } as GluehweinErlebnis;
    return enrichLookupFields([record], 'gluehwein_erlebnis')[0];
  }
  static async createGluehweinErlebni(fields: CreateGluehweinErlebnis) {
    return callApi('POST', `/apps/${APP_IDS.GLUEHWEIN_ERLEBNIS}/records`, { fields: cleanFieldsForApi(fields as any, 'gluehwein_erlebnis') });
  }
  static async updateGluehweinErlebni(id: string, fields: Partial<CreateGluehweinErlebnis>) {
    return callApi('PATCH', `/apps/${APP_IDS.GLUEHWEIN_ERLEBNIS}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'gluehwein_erlebnis') });
  }
  static async deleteGluehweinErlebni(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GLUEHWEIN_ERLEBNIS}/records/${id}`);
  }
