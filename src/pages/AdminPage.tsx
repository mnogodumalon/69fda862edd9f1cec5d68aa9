...
const GLUEHWEINERLEBNIS_FIELDS = [
  { key: 'datum', label: 'Datum & Uhrzeit', type: 'date/datetimeminute' },
  { key: 'ort', label: 'Ort / Veranstaltung', type: 'string/text' },
  { key: 'rezeptname', label: 'Rezeptname / Bezeichnung', type: 'string/text' },
  { key: 'zutaten', label: 'Zutaten', type: 'string/textarea' },
  { key: 'zubereitung', label: 'Zubereitung', type: 'string/textarea' },
  { key: 'weinsorte', label: 'Weinsorte', type: 'lookup/radio', options: [{ key: 'weisswein', label: 'Weißwein' }, { key: 'rose', label: 'Rosé' }, { key: 'alkoholfrei', label: 'Alkoholfrei' }, { key: 'rotwein', label: 'Rotwein' }] },
  { key: 'bewertung', label: 'Bewertung', type: 'lookup/radio', options: [{ key: 'stern_1', label: '⭐ (1 Stern)' }, { key: 'stern_2', label: '⭐⭐ (2 Sterne)' }, { key: 'stern_3', label: '⭐⭐⭐ (3 Sterne)' }, { key: 'stern_4', label: '⭐⭐⭐⭐ (4 Sterne)' }, { key: 'stern_5', label: '⭐⭐⭐⭐⭐ (5 Sterne)' }] },
  { key: 'besonderheiten', label: 'Besonderheiten', type: 'multiplelookup/checkbox', options: [{ key: 'sehr_suess', label: 'Sehr süß' }, { key: 'wuerzig', label: 'Würzig' }, { key: 'fruchtig', label: 'Fruchtig' }, { key: 'scharf', label: 'Scharf' }, { key: 'cremig', label: 'Cremig' }, { key: 'traditionell', label: 'Traditionell' }] },
  { key: 'notizen', label: 'Persönliche Notizen', type: 'string/textarea' },
  { key: 'foto', label: 'Foto', type: 'file' },
  { key: 'standort', label: 'Standort', type: 'geo' },
];

const ENTITY_TABS = [
  { key: 'gluehwein_erlebnis', label: 'Glühwein Erlebnis', pascal: 'GluehweinErlebnis' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('gluehwein_erlebnis');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'gluehwein_erlebnis': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gluehwein_erlebnis': return (data as any).gluehweinErlebnis as GluehweinErlebnis[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gluehwein_erlebnis': return GLUEHWEINERLEBNIS_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gluehwein_erlebnis': return {
        create: (fields: any) => LivingAppsService.createGluehweinErlebni(fields),
        update: (id: string, fields: any) => LivingAppsService.updateGluehweinErlebni(id, fields),
        remove: (id: string) => LivingAppsService.deleteGluehweinErlebni(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'gluehwein_erlebnis' || dialogState?.entity === 'gluehwein_erlebnis') && (
        <GluehweinErlebnisDialog
          open={createEntity === 'gluehwein_erlebnis' || dialogState?.entity === 'gluehwein_erlebnis'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'gluehwein_erlebnis' ? handleUpdate : (fields: any) => handleCreate('gluehwein_erlebnis', fields)}
          defaultValues={dialogState?.entity === 'gluehwein_erlebnis' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['GluehweinErlebnis']}
          enablePhotoLocation={AI_PHOTO_LOCATION['GluehweinErlebnis']}
        />
      )}
      {viewState?.entity === 'gluehwein_erlebnis' && (
        <GluehweinErlebnisViewDialog
          open={viewState?.entity === 'gluehwein_erlebnis'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'gluehwein_erlebnis', record: r }); }}
        />
      )}
