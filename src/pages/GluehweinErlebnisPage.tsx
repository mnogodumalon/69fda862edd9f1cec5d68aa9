...
import { GluehweinErlebnisViewDialog } from '@/components/dialogs/GluehweinErlebnisViewDialog';
...
export default function GluehweinErlebnisPage() {
  const [records, setRecords] = useState<GluehweinErlebnis[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<GluehweinErlebnis | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<GluehweinErlebnis | null>(null);
  const [viewingRecord, setViewingRecord] = useState<GluehweinErlebnis | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getGluehweinErlebnis());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: GluehweinErlebnis['fields']) {
    await LivingAppsService.createGluehweinErlebni(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: GluehweinErlebnis['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateGluehweinErlebni(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Glühwein Erlebnis"
      subtitle={`${records.length} Glühwein Erlebnis im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Glühwein Erlebnis suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('rezeptname')}>
                <span className="inline-flex items-center gap-1">
                  Rezeptname / Bezeichnung
                  {sortKey === 'rezeptname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('zutaten')}>
                <span className="inline-flex items-center gap-1">
                  Zutaten
                  {sortKey === 'zutaten' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('weinsorte')}>
                <span className="inline-flex items-center gap-1">
                  Weinsorte
                  {sortKey === 'weinsorte' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.rezeptname ?? '—'}</TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.zutaten ?? '—'}</span></TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.zubereitung ?? '—'}</span></TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.weinsorte?.label ?? '—'}</span></TableCell>
                ...
                <TableCell>{Array.isArray(record.fields.besonderheiten) ? record.fields.besonderheiten.map((v: any) => v?.label ?? v).join(', ') : '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={12} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Glühwein Erlebnis. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Glühwein Erlebnis löschen"
        description="Soll dieser Eintrag wirklich gelöscht werden? Diese Aktion kann nicht rückgängig gemacht werden."
      />

      <GluehweinErlebnisViewDialog
        open={!!viewingRecord}
