...
interface GluehweinErlebnisDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: GluehweinErlebnis['fields']) => Promise<void>;
  defaultValues?: GluehweinErlebnis['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function GluehweinErlebnisDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: GluehweinErlebnisDialogProps) {
  const [fields, setFields] = useState<Partial<GluehweinErlebnis['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'gluehwein_erlebnis');
      await onSubmit(clean as GluehweinErlebnis['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "datum": string | null, // YYYY-MM-DDTHH:MM\n  "ort": string | null, // Ort / Veranstaltung\n  "rezeptname": string | null, // Rezeptname / Bezeichnung\n  "zutaten": string | null, // Zutaten\n  "zubereitung": string | null, // Zubereitung\n  "weinsorte": LookupValue | null, // Weinsorte (select one key: "weisswein" | "rose" | "alkoholfrei" | "rotwein") mapping: weisswein=Weißwein, rose=Rosé, alkoholfrei=Alkoholfrei, rotwein=Rotwein\n  "bewertung": LookupValue | null, // Bewertung (select one key: "stern_1" | "stern_2" | "stern_3" | "stern_4" | "stern_5") mapping: stern_1=⭐ (1 Stern), stern_2=⭐⭐ (2 Sterne), stern_3=⭐⭐⭐ (3 Sterne), stern_4=⭐⭐⭐⭐ (4 Sterne), stern_5=⭐⭐⭐⭐⭐ (5 Sterne)\n  "besonderheiten": LookupValue[] | null, // Besonderheiten (select one or more keys: "sehr_suess" | "wuerzig" | "fruchtig" | "scharf" | "cremig" | "traditionell") mapping: sehr_suess=Sehr süß, wuerzig=Würzig, fruchtig=Fruchtig, scharf=Scharf, cremig=Cremig, traditionell=Traditionell\n  "notizen": string | null, // Persönliche Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<GluehweinErlebnis['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Glühwein Erlebnis bearbeiten' : 'Glühwein Erlebnis hinzufügen';
